...
function _backupTodayOnce(){  // 하루 1회, 그날 첫 접속 시점의 상태를 서버에 백업 보관 (stateBackups/YYYYMMDD)
  try{
    if(typeof students==='undefined' || !students || !students.length) return;  // 빈 상태는 백업 안 함
    const t=new Date();
    const key=`${t.getFullYear()}${String(t.getMonth()+1).padStart(2,'0')}${String(t.getDate()).padStart(2,'0')}`;
    const ref=fbDb.collection('stateBackups').doc(key);
    ref.get().then(s=>{ if(!s.exists) ref.set(JSON.parse(_lastJSON)).catch(()=>{}); }).catch(()=>{});
  }catch(e){}
}
...
async function loadData(){
  try{
    ...
    _loadFailed=true;    // ★ 로드 실패 = 이 세션에서는 어떤 저장도 하지 않음
    try{ document.body.insertAdjacentHTML('beforeend',
      '<div style="position:fixed;left:0;right:0;top:0;z-index:99999;background:#A32D2D;color:#fff;padding:10px;text-align:center;font-size:14px;font-family:inherit">⚠ 데이터를 불러오지 못했습니다. 인터넷 연결을 확인하고 새로고침 해주세요. (안전을 위해 저장이 차단됩니다)</div>'); }catch(e2){}
  }
